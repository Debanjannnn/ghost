...
function readEthPrice(runtime2) {
  const net = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.feedChainName,
    isTestnet: false
  });
  if (!net)
    throw new Error(`Network not found: ${runtime2.config.feedChainName}`);
  const evmClient = new cre.capabilities.EVMClient(net.chainSelector.selector);
  const feedAddr = runtime2.config.ethUsdFeed;
  const decData = encodeFunctionData({
    abi: PriceFeedAggregator,
    functionName: "decimals"
  });
  const decResp = evmClient.callContract(runtime2, {
  ...
  const ansResp = evmClient.callContract(runtime2, {
