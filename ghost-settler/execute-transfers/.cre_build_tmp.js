...
class IntegerOutOfRangeError extends BaseError {
  constructor({ max, min, signed, size, value: value2 }) {
    super(`Number "${value2}" is not in safe ${size ? `${size * 8}-bit ${signed ? "signed" : "unsigned"} ` : ""}integer range ${max ? `(${min} to ${max})` : `(above ${min})`}`, { name: "IntegerOutOfRangeError" });
  }
}
...
function keccak256(value2, to_) {
  const to = to_ || "hex";
  const bytes = keccak_256(isHex(value2, { strict: false }) ? toBytes2(value2) : value2);
  if (to === "bytes")
    return bytes;
  return toHex(bytes);
}
...
function sha2563(value2, to_) {
  const to = to_ || "hex";
  const bytes = sha2562(isHex(value2, { strict: false }) ? toBytes2(value2) : value2);
  if (to === "bytes")
    return bytes;
  return toHex(bytes);
}
...
function assertTransactionEIP4844(transaction) {
  ...
  if (blobVersionedHashes) {
    ...
    for (const hash of blobVersionedHashes) {
      ...
      const version2 = hexToNumber(slice2(hash, 0, 1));
      if (size_ !== 32)
...
function hashDomain({ domain, types: types4 }) {
  return hashStruct({
    data: domain,
...
function hashStruct({ data, primaryType, types: types4 }) {
...
function encodeData({ data, primaryType, types: types4 }) {
  const encodedTypes = [{ type: "bytes32" }];
  const encodedValues = [hashType({ primaryType, types: types4 })];
...
function hashType({ primaryType, types: types4 }) {
  const encodedHashType = toHex(encodeType({ primaryType, types: types4 }));
  return keccak256(encodedHashType);
}
function encodeType({ primaryType, types: types4 }) {
  let result = "";
  const unsortedDeps = findTypeDependencies({ primaryType, types: types4 });
  unsortedDeps.delete(primaryType);
  const deps = [primaryType, ...Array.from(unsortedDeps).sort()];
  for (const type of deps) {
    result += `${type}(${types4[type].map(({ name, type: t }) => `${t} ${name}`).join(",")})`;
  }
  return result;
}
function findTypeDependencies({ primaryType: primaryType_, types: types4 }, results = new Set) {
  ...
  for (const field of types4[primaryType]) {
    findTypeDependencies({ primaryType: field.type, types: types4 }, results);
  }
  return results;
}
function encodeField({ types: types4, name, type, value: value2 }) {
...
var onCronTrigger = async (runtime2, _payload) => {
  ...
  if (!poolPrivateKey)
    return "error:no-pool-key";
  const account = privateKeyToAccount(poolPrivateKey.startsWith("0x") ? poolPrivateKey : `0x${poolPrivateKey}`);
  const domain = getDomain(runtime2.config);
  const executedIds = [];
  const failedIds = [];
  for (const transfer of transfers) {
    try {
      ...
      const execResp = confClient.sendRequest(runtime2, {
        vaultDonSecrets: [],
        request: {
          url: runtime2.config.externalApiUrl + "/private-transfer",
          method: "POST",
          multiHeaders: {
  ...
  const confirmResp = confClient.sendRequest(runtime2, {
    vaultDonSecrets: API_KEY_SECRET,
    request: {
  ...
  if (!ok(confirmResp))
    return "error:confirm-failed executed=" + executedIds.length;
  const result = "executed=" + executedIds.length + " failed=" + failedIds.length;
  runtime2.log("execute-transfers result: " + result);
  return result;
};
